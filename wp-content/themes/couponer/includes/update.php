...
if (is_admin()){
	...
	function powerthemes_update_page() {
		add_menu_page(__( 'PowerThemes Updates', 'couponer' ), __( 'PowerThemes Updates', 'couponer' ), 'manage_options', 'powerthemes-updates', 'powerthemes_update_settings', 'dashicons-update');
	}	
	...
	function powerthemes_update_settings(){
		...
		echo '<div class="wrap">
    			...
    			<div class="narrow">
    				<p>'.__( 'Input your data to receive information about theme updates', 'couponer' ).'.</p>
				</div>';
		echo $message;
		if( empty( $envato_username ) && empty( $envato_api ) && empty( $purchase_code ) ){
						...
						<table class="form-table">
							<tbody>
								<tr class="form-field form-required">
									<th scope="row"><label for="envato_username">'.__( 'Username', 'couponer' ).' <span class="description">('.__( 'required', 'couponer' ).')</span></label></th>
									<td><input name="envato_username" type="text" id="envato_username" value="'.esc_attr( $envato_username ).'" aria-required="true"></td>
								</tr>
								<tr class="form-field form-required">
									<th scope="row"><label for="envato_api">'.__( 'API', 'couponer' ).' <span class="description">('.__( 'required', 'couponer' ).')</span></label></th>
									<td><input name="envato_api" type="text" id="envato_api" value="'.esc_attr( $envato_api ).'"></td>
								</tr>
								<tr class="form-field form-required">
									<th scope="row"><label for="purchase_code">'.__( 'Purchase Code', 'couponer' ).' <span class="description">('.__( 'required', 'couponer' ).')</span></label></th>
									<td><input name="purchase_code" type="text" id="purchase_code" value="'.esc_attr( $purchase_code ).'"></td>
								</tr>
							</tbody>
						</table>
						<p><input type="submit" value="'.esc_attr__( 'Connect', 'couponer' ).'" class="button button-primary"></p>
					</form>';
		}
		else{
			...
			echo '<table class="form-table">
				...
				<p><a href="'.esc_url( add_query_arg( array( 'deactivate' => '1' ), "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]" ) ).'" class="button button-primary">'.esc_attr__( 'Disconnect', 'couponer' ).'</a></p>';
		}
		echo '</div>';
	}	
}
