...
if ( ! class_exists('NHP_Options') ){
	...
	if(!defined('NHP_OPTIONS_URL')){
		define('NHP_OPTIONS_URL', plugin_dir_url( __FILE__ ));
	}
...
class NHP_Options{
	...
	*/
	function _enqueue(){
		...
		wp_enqueue_style(
			'nhp-tooltipster',
			apply_filters('nhp-opts-ui-theme', $this->url.'css/tooltipster.css'),
			'',
			time(),
			'all'
		);		
		...
		wp_enqueue_script(
			'nhp-tooltipster-js', 
			$this->url.'js/jquery.tooltipster.js', 
			array('jquery'),
			time(),
			true
		);
			...
			if(isset($section['fields'])){
				...
				foreach($section['fields'] as $fieldk => $field){
					...
					if(isset($field['title'])){
						...
						if( isset( $field['image_desc'] ) || isset( $field['text_desc'] ) ){
							if( isset( $field['text_desc'] ) ){
								$field['text_desc'] = '<div class="nhp_info">'.$field['text_desc'].'</div>';
							}
							$th = '<span class="dashicons dashicons-info nhp-help" title="'.( isset( $field['image_desc'] ) ? '&lt;img src=&quot;'.$field['image_desc'].'&quot; class=&quot;nhp_image_info&quot; &gt;'.htmlspecialchars( $field['text_desc'] ) : '' ).'"></span>'.$th;
						}
					}else{
						$th = '';
					}
